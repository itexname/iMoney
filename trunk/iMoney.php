...
class itex_money
{
	...
	function itex_m_safe_url()
	{
		...
		$url=explode("?",strtolower($this->server['REQUEST_URI']));
		if(isset($url[1]))
		{
    ...
    * @return  string $ret  
	...
	function itex_m_init()
	{
		...
		if ($this->get_option('itex_m_global_masking'))
		{
			...
			$last_REQUEST_URI = $this->server['REQUEST_URI'];
			$this->server['REQUEST_URI'] = $this->safeurl;
		}
		$this->itex_debug('REQUEST_URI = '.$this->server['REQUEST_URI']);
		...
		{
			$this->server['REQUEST_URI'] = $last_REQUEST_URI;
	...
	function itex_init_sape()
	{
		...
		if ($this->get_option('itex_m_sape_enable'))
		{
			...
			///check it
			$url = 1;
			if ($this->wordpress) if (is_object($GLOBALS['wp_rewrite'])) $url = url_to_postid($this->server['REQUEST_URI']);
		...
		if ($this->get_option('itex_m_sape_sapearticles_enable'))
		{
			...
			if(is_callable(array('SAPE_articles', 'SAPE_articles')))
			{
				...
				if (strlen($preg) > 4)
				{
					...
					if (preg_match($preg,$this->server['REQUEST_URI']))
					{
						$isvalidurl = 1;
					}
				}
				...
				//генерация шаблона
				if (preg_match('@itex_sape_articles_template\.'._SAPE_USER.'@i',$this->server['REQUEST_URI']))
				{
					if (!headers_sent())
					{
						header(200);
...
//--></script><script type="text/javascript" src="http://autocontext.begun.ru/autocontext2.js"></script></p>';
    ...
    * @return  bool
	...
	function itex_init_ilinks()
	{
		...
		if ($this->get_option('itex_m_ilinks_sidebar_enable'))
		{
			...
			foreach ($l as $q)
			{
				...
				if (strtolower($w[0]{0}) == 'r')
				{
					...
					if (preg_match("@".$w[0]."@i",$this->server["REQUEST_URI"])) $this->sidebar['iMoney_ilinks']  .= $w[1];
				}
				elseif  ($this->server["REQUEST_URI"] == $w[0]) $this->sidebar['iMoney_ilinks']  .= $w[1];
		...
		if ($this->get_option('itex_m_ilinks_footer_enable'))
		{
			...
			foreach ($l as $q)
			{
				...
				{
					$w[0] = substr($w[0],1,strlen($w[0]));
					//if (eregi($w[0],$this->server["REQUEST_URI"])) $this->footer .= $w[1];
					if (preg_match("@".$w[0]."@i",$this->server["REQUEST_URI"])) $this->footer .= $w[1];

				}
				elseif  ($this->server["REQUEST_URI"] == $w[0]) $this->footer .= $w[1];
			}
		}
		if ($this->get_option('itex_m_ilinks_beforecontent_enable'))
		{
			...
			foreach ($l as $q)
			{
				...
				{
					$w[0] = substr($w[0],1,strlen($w[0]));
					//if (eregi($w[0],$this->server["REQUEST_URI"])) $this->beforecontent .= $w[1];
					if (preg_match("@".$w[0]."@i",$this->server["REQUEST_URI"])) $this->beforecontent .= $w[1];

				}
				elseif  ($this->server["REQUEST_URI"] == $w[0]) $this->beforecontent .= $w[1];
			}
		}
		if ($this->get_option('itex_m_ilinks_aftercontent_enable'))
		{
			...
			foreach ($l as $q)
			{
				...
				{
					$w[0] = substr($w[0],1,strlen($w[0]));
					//if (eregi($w[0],$this->server["REQUEST_URI"])) $this->aftercontent .= $w[1];
					if (preg_match("@".$w[0]."@i",$this->server["REQUEST_URI"])) $this->aftercontent .= $w[1];
				}
				elseif  ($this->server["REQUEST_URI"] == $w[0]) $this->aftercontent .= $w[1];
			}
		}
		return true;
	}
	...
	function itex_m_replace($content)
	{
		...
		if ($this->get_option('itex_m_sape_sapecontext_enable'))
		{
			if (url_to_postid($this->server['REQUEST_URI']) || !$this->get_option('itex_sape_pages_enable'))
			{
			...
			else $this->itex_debug('url_to_postid='.url_to_postid($this->server['REQUEST_URI']).' itex_sape_pages_enable='.$this->get_option('itex_sape_pages_enable'));
		}
    ...
    */
	...
	function itex_m_admin_sape()
	{
		...
		if ($this->get_option('itex_m_sape_sapeuser'))
		{
			...
			{
				$file = str_replace($this->server["SCRIPT_NAME"],'',$this->server["SCRIPT_FILENAME"]).'/'.$this->get_option('itex_m_sape_sapeuser').'/sape.php';
				if (file_exists($file)) {}
			else {?>
		<div style="margin:10px auto; border:3px #f00 solid; background-color:#fdd; color:#000; padding:10px; text-align:center;">
				Sape dir not exist!
	...
	function itex_m_admin_zilla()
	{
		...
		if ($this->get_option('itex_m_zilla_user'))
		{
			...
			{
				$file = str_replace($this->server["SCRIPT_NAME"],'',$this->server["SCRIPT_FILENAME"]).'/'.$this->get_option('itex_m_zilla_user').'/zilla.php';
				if (file_exists($file)) {}
			else {?>
		<div style="margin:10px auto; border:3px #f00 solid; background-color:#fdd; color:#000; padding:10px; text-align:center;">
	...
	function itex_m_admin_trustlink()
	{
		...
		if ($this->get_option('itex_m_trustlink_user'))
		{
			...
			{
				$file = str_replace($this->server["SCRIPT_NAME"],'',$this->server["SCRIPT_FILENAME"]).'/'.$this->get_option('itex_m_trustlink_user').'/trustlink.php';
				if (file_exists($file)) {}
			else {?>
		<div style="margin:10px auto; border:3px #f00 solid; background-color:#fdd; color:#000; padding:10px; text-align:center;">
    ...
    */
	...
	function itex_m_tnx_install_file()
	{
		...
		$file = str_replace($this->server["SCRIPT_NAME"],'',$this->server["SCRIPT_FILENAME"]).'/'.'tnxdir_'.md5($this->get_option('itex_m_tnx_tnxuser')).'/tnx.php';
	...
	function itex_m_admin_setlinks()
	{
		...
		if ($this->get_option('itex_m_setlinks_setlinksuser'))
		{
			...
			{
				$file = str_replace($this->server["SCRIPT_NAME"],'',$this->server["SCRIPT_FILENAME"]).'/setlinks_'.$this->get_option('itex_m_setlinks_setlinksuser').'/';
				if (is_dir($file)) {}
			else {?>
		<div style="margin:10px auto; border:3px #f00 solid; background-color:#fdd; color:#000; padding:10px; text-align:center;">
